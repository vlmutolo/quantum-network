...
def create_overlay_on_non_wrap_grid_graph(G, initial_capacity, rows, cols):
...
def create_overlay_on_grid_graph(G, initial_capacity, rows, cols):
...
def create_graph(n, initial_capacity, graph_type, edges_with_consumption_non_zero_rate):
    G = nx.Graph()
    ...
    if graph_type == "cycle":
        ...
        G = create_overlay_on_grid_graph(G, initial_capacity, rows, cols)
        pos = {(i, j): (j, -i) for i in range(rows) for j in range(cols)}  # y = -i to plot top-to-bottom


    nodes = [(i, j) for i in range(rows) for j in range(cols)] if "grid" in graph_type else list(range(n))
...
def is_stabilized_overlay(arr, num_last=9, tolerance=0.2, nodes = 20):
    if (nodes <= 30):
        num_last = 15
        tolerance = 0.05
    elif (nodes <= 50):
...
def is_stabilized_efficiency(arr, num_last=10, tolerance=0.5, nodes = 20):
    if (nodes <= 30):
        num_last = 15
        tolerance = 0.1
    elif (nodes <= 50):
    ...
    for t in range(1, time_steps + 1):
        if event == "generate":
            ...
            if (distillation_pairs == 1):
               bell_pairs_generated = np.random.poisson(5)
        ...
        if t % 100 == 0:
            if (t % 1000 == 0):
                print (t, swap_overlay[-7:], swap_efficiency[-7:], succeeded, len(nodes))
            if optimal_swap_count > 0 and succeeded >0 :
                ...
                if is_stabilized_overlay(swap_overlay, 9, 0.5, len(nodes)) and is_stabilized_efficiency(swap_efficiency, 9, 0.5, len(nodes)):
...
def main():
    ...
    for graph_type in graph_types:
        nodes = list(range(20, 71, 10))
        
        print(f"Simulating node increase for {graph_type}")
        sim_args = []
        for n in nodes:
            edges_with_consumption_non_zero_rate = math.floor(0.1 * n * (n - 1) / 2)
            G, consumption_rates = create_graph(n, initial_capacity, graph_type, edges_with_consumption_non_zero_rate)
            sim_args.append((G, consumption_rates, 5_000_000, generation_rate, 200, 1, True))
        ...
        df.to_csv(f"{graph_type}_node_metrics_output-distillation-1.csv", index=False)
    ...
    for graph_type in graph_types:
        ...
        for n in nodes:
            edges_with_consumption_non_zero_rate = math.floor(0.1 * n * (n - 1) / 2)
            G, consumption_rates = create_graph(n, initial_capacity, graph_type, edges_with_consumption_non_zero_rate)
            sim_args.append((G, consumption_rates, 5_000_000, generation_rate, 200, 2, True))
        ...
        df.to_csv(f"{graph_type}_node_metrics_output-distillation-2.csv", index=False)
