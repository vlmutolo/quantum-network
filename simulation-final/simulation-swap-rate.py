...
def is_stabilized(arr, num_last=20, tolerance=0.04):
...
def run_simulation(args):
    ...
    return simulate_consumption_sequence(G.copy(), consumption_rates.copy(), time_steps, generation_rate, swap_rate, distillation_pairs, infinite_swapping_enabled)


def simulate_consumption_sequence(G, consumption_rates, time_steps, generation_rate, swap_rate, distillation_pairs, infinite_swapping_enabled):
    ...
    for t in range(1, time_steps + 1):
        ...
        if event == "generate":
            ...
            if (distillation_pairs <= 2):
               bell_pairs_generated = np.random.poisson(3)
            elif (distillation_pairs <= 5): 
                bell_pairs_generated = np.random.poisson(10)
            elif (distillation_pairs > 5): 
                bell_pairs_generated = np.random.poisson(20)
        ...
        if t % 20000 == 0:
            if (t % 100000 == 0):
                print (t, swap_overlay[-5:], swap_efficiency[-5:])
            if optimal_swap_count > 0 and succeeded >0 :
                ...
                if is_stabilized(swap_overlay) and is_stabilized(swap_efficiency):
...
def main():
    ...
    for graph_type in graph_types:
        ...
        swap_rates = list(range(150, 410, 50))
        
        sim_args = [(G, consumption_rates, 5_000_000, generation_rate, rate, 1, True) for rate in swap_rates]

        with Pool(processes=min(2, len(sim_args))) as pool:
        ...
        df.to_csv(f"{graph_type}_swap_rate_metrics_pool_output.csv", index=False)
        ...
        df.to_csv(f"{graph_type}_distillation_metrics_pool_output.csv", index=False)
